import torch
from stage1.audio_to_mel import Audio2Mel
...
def total_loss(fmap_real, logits_fake, fmap_fake, input_wav, output_wav, sample_rate=16000):
    ...
    # pad_size1 = input_wav.shape[2] % 320  # 需要填充的长度
    # padding = torch.zeros(input_wav.size(0), input_wav.size(1),pad_size1, device=input_wav.device)  # 创建填充的 0 张量
    # input_wav = torch.cat([input_wav, padding], dim=-1)  # 在最后一维上拼接填充
